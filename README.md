...
- **自然语言提醒 (/remind)** — 普通用户即可通过 `/remind 自然语言` 创建一次性或重复提醒，LLM 自动解析「明天早上 9 点交报销」「每周一 9 点开会」等输入；提醒与调度器共用 tick，但条目用户私有、`/reminders` 只列出自己的、`/reminder_del` 只能删自己的；唯一入口是显式的 `/remind*` 命令，聊天流与工具调用层无法读写提醒存储
...
与 `/schedule` 面向管理员的 cron+JSON 定义不同，`/remind` 面向所有允许用户，用自然语言创建一次性或重复提醒：

- 用户发送 `/remind 明天早上9点提醒我交报销`，机器人调用解析 LLM 将其转换为 `{fire_at, cron?, prompt}`，写入 `chat.db` 的 `user_reminders` bucket
- 触发时与 `/schedule` 共用同一个 15s 调度 tick，一次性提醒触发即删除，重复提醒按 cron 自动重排
- 触发后通过 `startChatFlow` 以合成用户消息的形式进入正常回复链路，享受完整的历史、摘要、工具、贴纸等能力
- **安全边界**：仅 `/remind` `/reminders` `/reminder_del` 三个命令能访问提醒存储，聊天流/工具调用层完全隔离。即使模型被 jailbreak，也无法创建或删除提醒
- **可见范围**：群聊中 `/reminders` 只列出本群聊内由当前用户创建的提醒；私聊中列出当前用户在任何聊天创建的全部提醒。提醒始终在创建所在的 chat 触发

#### Reminder 命令

| 命令 | 说明 |
|---|---|
| `/remind <自然语言>` | 创建一次性或重复提醒 |
| `/reminders` | 列出自己的提醒（群聊仅限本群聊；私聊列出全部） |
| `/reminder_del <id>` | 删除指定 ID 的提醒（仅能删自己的） |
| `/reminderd <id>` | `/reminder_del` 的别名 |
...
| 字段 | 说明 |
|---|---|
| `REMINDER_DEFAULT_TIMEZONE` | 解析器使用的默认 IANA 时区（默认 `Asia/Shanghai`） |
| `REMINDER_API_BASE` | 可选：解析器专用 API base，为空则回退主 OpenAI 客户端 |
| `REMINDER_API_KEY` | 可选：解析器专用 API key |
| `REMINDER_MODEL` | 可选：解析器专用 model，为空则回退 `OPENAI_MODEL` |

所有配置均已接入 `/admin` 热修改面板，`REMINDER_DEFAULT_TIMEZONE` 设置时会验证 IANA 时区有效性。
...
| `/remind <自然语言>` | 创建一次性或重复提醒（所有允许用户） |
| `/reminders` | 列出自己的提醒 |
| `/reminder_del <id>` | 删除自己的提醒（`/reminderd` 为别名） |
...
remind - 自然语言创建一次性或重复提醒
reminders - 列出自己的提醒
reminder_del - 删除自己的提醒
admin - 管理员配置面板
```
...
用户文档 → handleDocument()
            ├─ 权限检查: isAllowed()
            ├─ DOCUMENT_ENABLED=false → 静默忽略
            ├─ 扩展名白名单校验（DOCUMENT_ALLOWED_EXTS）→ 不在白名单 → 拒绝
            ├─ 群聊: 检查 caption 中的 @mention（或 AUTO_DETECT 判定相关）
            ├─ 下载 → io.LimitReader 二次加界 → 校验不超过 DOCUMENT_MAX_BYTES
            ├─ 文本提取: PDF → pdftotext (stdin/stdout, 30s 超时)
            │                其余 → UTF-8 读取（非法字节替换为 U+FFFD）
            ├─ 按 rune 硬截断到 DOCUMENT_MAX_TEXT_CHARS
            └─ 构建 userMsg（Content 含文件元信息 + 截断文本）→ startChatFlow()
